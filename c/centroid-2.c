...
void dfs(Diag* diag, int n, int root, int par, int* node_size, int* max_node) {
    node_size[root] = 1;
    for (int nxt = diag->head[root]; nxt != 0; nxt = diag->next[nxt]) {
        if (diag->to[nxt] != par) {
            dfs(diag, n, diag->to[nxt], root, node_size, max_node);
            node_size[root] += node_size[diag->to[nxt]];
            if (node_size[diag->to[nxt]] > max_node[root]) {
                max_node[root] = node_size[diag->to[nxt]];
            }
        }
    }
    int rem = n - node_size[root];
    if (max_node[root] < rem) {
        max_node[root] = rem;
    }
}
