use std::io::{self, BufRead, BufReader, BufWriter, Write};

fn main() {
    let mut br = BufReader::new(io::stdin().lock());
    let mut bw = BufWriter::new(io::stdout().lock());
    let mut buf = String::new();
    br.read_line(&mut buf).unwrap();
    let mut segs = buf.trim().split_whitespace();
    let n: usize = segs.next().unwrap().parse().unwrap();
    let m: usize = segs.next().unwrap().parse().unwrap();
    let mut st = STAddMax::new(n);
    for _ in 0..m {
        buf.clear();
        br.read_line(&mut buf).unwrap();
        segs = buf.trim().split_whitespace();
        let op: usize = segs.next().unwrap().parse().unwrap();
        let r1: usize = segs.next().unwrap().parse().unwrap();
        let r2: usize = segs.next().unwrap().parse().unwrap();
        if op == 0 {
            st.switch(r1, r2, 1, n, 1);
        } else {
            bw.write_fmt(format_args!("{}\n", st.query(r1, r2, 1, n, 1)))
                .unwrap()
        }
    }
    bw.flush().unwrap()
}

fn m(l: usize, r: usize) -> usize {
    l + ((r - l) >> 1)
}

struct STAddMax {
    info: Vec<usize>,
    switch: Vec<bool>,
}

impl STAddMax {
    fn new(n: usize) -> Self {
        Self {
            info: vec![0; n << 2],
            switch: vec![false; n << 2],
        }
    }

    fn up(&mut self, i: usize) {
        self.info[i] = self.info[i << 1] + self.info[i << 1 | 1]
    }

    // 将某个位置的懒信息传递下去
    fn down(&mut self, i: usize, ln: usize, rn: usize) {
        // 如果有懒信息
        if self.switch[i] {
            self.lazy(i << 1, ln);
            self.lazy(i << 1 | 1, rn);
            // 重置当前的懒信息
            self.switch[i] = false;
        }
    }

    // 懒信息要加到某个范围上，需要同步更新结果信息
    fn lazy(&mut self, i: usize, n: usize) {
        self.info[i] = n - self.info[i];
        self.switch[i] = !self.switch[i];
    }

    fn switch(&mut self, jobl: usize, jobr: usize, l: usize, r: usize, i: usize) {
        // 任务范围完全包括当前范围，那么整个范围只要累积懒信息即可
        if jobl <= l && jobr >= r {
            self.lazy(i, r - l + 1);
        } else {
            let m = m(l, r);
            // 先将当前的懒信息移走，然后再在子范围上添加这个数字
            self.down(i, m - l + 1, r - m);
            if jobl <= m {
                self.switch(jobl, jobr, l, m, i << 1);
            }
            if jobr > m {
                self.switch(jobl, jobr, m + 1, r, i << 1 | 1);
            }
            // ⚠️子范围调整完之后要更新父范围
            self.up(i);
        }
    }

    fn query(&mut self, jobl: usize, jobr: usize, l: usize, r: usize, i: usize) -> usize {
        if jobl <= l && jobr >= r {
            self.info[i]
        } else {
            let m = m(l, r);
            self.down(i, m - l + 1, r - m);
            let mut res = 0;
            if jobl <= m {
                res += self.query(jobl, jobr, l, m, i << 1);
            }
            if jobr > m {
                res += self.query(jobl, jobr, m + 1, r, i << 1 | 1);
            }
            res
        }
    }
}
